...
def show_patient(data, SLICE_NUMBER=1, train=True, test=False):
    ...
    dataset = view_train_patient if train == True else view_test_patient

    plt.figure("Visualization Train" if train == True else "Visualization Test", (12, 6))
    plt.subplot(1, 2, 1)
    plt.title(f"vol {SLICE_NUMBER}")
    plt.imshow(dataset["vol"][0, 0, :, :, SLICE_NUMBER], cmap="gray")

    plt.subplot(1, 2, 2)
    plt.title(f"seg {SLICE_NUMBER}")
    plt.imshow(dataset["seg"][0, 0, :, :, SLICE_NUMBER])
    plt.show()
